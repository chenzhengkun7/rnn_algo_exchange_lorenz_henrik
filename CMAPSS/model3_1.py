...
norm_cyc = train_df.iloc[:, 1]
train_df.columns = columns = ['id', 'cycle', 's2', 's3', 's4', 's11', 's15']
...
norm_cyc = norm_cyc.iloc[0:(len(train_df)), ]
...
# build lstm-model
model = Sequential()
model.add(LSTM(150, input_shape=(20, 2)))
model.add(Dense(5))
